...
export const FollowingPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    async function loadFollowing() {
      ...
      try {
        ...
        const following = await socialGraphApi.getFollowing(profileId);
  ...
  if (loading) {
    return (
      <div className="flex flex-col gap-6">
        <PageHeader title="Following" description="Loading connections list..." />
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        title={`${profileName}'s Following`}
        description={isOwnProfile ? 'Manage the connections you are following.' : `Viewing users followed by @${profile.username}.`}
        backLink={
          <Link
            to={basePath}
      ...
      <PeopleList
        people={people}
        emptyLabel={isOwnProfile ? 'You are not following anyone yet. Search users to build your network!' : `@${profile.username} is not following anyone yet.`}
      />
    </div>
  );
};
